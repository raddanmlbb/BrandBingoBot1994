...
async def game_type_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    global game_active, players, bingo_history, history_msg_id, progress_msg_id, game_vip_mode, registration_open, current_winner, game_paused
    game_active = True
    players.clear()
    bingo_history.clear()
    history_msg_id = None
...
async def stopgame(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.username.lower() not in [a.lower() for a in ADMIN_USERNAMES]:
        await update.message.reply_text("❌ Только администратор.")
        return
    global game_active, players, bingo_history, history_msg_id, progress_msg_id, registration_open, current_winner, game_paused
    game_active = False
    players.clear()
    bingo_history.clear()
    history_msg_id = None
...
async def endgame(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    global current_winner, game_paused, game_active
    
    if not game_paused or not current_winner:
        await update.message.reply_text("Нет ожидающего решения победителя.")
        return
    
    args = context.args
    if not args or args[0].lower() not in ["yes", "no"]:
        await update.message.reply_text(
            "⚠️ Использование:\n"
            "/endgame yes — подтвердить победу\n"
            "/endgame no — отказать (игра продолжится без него)"
        )
        return
    
    winner_uid = current_winner["user_id"]
    winner_uname = current_winner["username"]
    all_winners = current_winner.get("all_winners", [(winner_uid, winner_uname)])
    ...
    if args[0].lower() == "yes":
        ...
        disqualified = players.pop(winner_uid, None)
        ...
        try:
            await context.bot.send_message(
                winner_uid,
                "😔 К сожалению, ваша победа не подтверждена администратором.\n"
                "Возможно, вы не выполнили условия розыгрыша.\n\n"
                "Не расстраивайтесь — участвуйте в следующих играх!"
            )
        except:
            pass
...
async def bingo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if game_paused:
        await update.message.reply_text(
            "⏸️ Игра на паузе. Сначала примите решение по текущему победителю:\n"
            "/endgame yes — подтвердить\n"
            "/endgame no — отказать"
        )
        return
    if not players:
    ...
    if winners:
        ...
        try:
            if len(winners) > 1:
                await context.bot.send_message(
                    winner_uid,
                    "🎯 Поздравляем! Все ваши числа выпали!\n\n"
                    "🎲 Среди нескольких победителей рандом выбрал именно вас.\n"
                    "⚖️ Ничего личного — просто повезло.\n\n"
                    "⚠️ Теперь финальное решение за администратором. Если вы выполнили все условия "
                    "розыгрыша — админ подтвердит победу командой /endgame yes.\n\n"
                    "Ожидайте..."
                )
            else:
                await context.bot.send_message(
                    winner_uid,
                    "🎯 Поздравляем! Все ваши числа выпали!\n\n"
                    "⚠️ Но финальное решение за администратором. Если вы выполнили все условия розыгрыша — "
                    "админ подтвердит вашу победу командой /endgame yes.\n\n"
                    "Ожидайте..."
                )
        except:
            pass
            ...
            for uid, uname in winners:
                if uid != winner_uid:
                    try:
                        await context.bot.send_message(
                            uid,
                            f"🎯 Вы тоже собрали все числа, но рандом выбрал другого участника.\n"
                            f"⚖️ Ничего личного — просто не повезло в этот раз.\n"
                            f"😔 Ваша победа не засчитана. Ждите следующую игру!"
                        )
                    except:
                        pass
...
async def handle_register(message, context, user_id):
    ...
    if not registration_open:
        await message.reply_text("Регистрация на эту игру уже закрыта. Ждите следующую.")
        return
    if db.is_banned(user_id):
        await message.reply_text("❌ Вы в чёрном списке и не можете участвовать.")
        return
    if game_paused:
        await message.reply_text("⏸️ Игра на паузе, дождитесь решения администратора по текущему победителю.")
        return
    if game_vip_mode:
...
async def handle_progress(message, context, user_id):
    if not players:
        await message.reply_text("Пока нет зарегистрированных участников.")
        return
    lines = []
    for uid,data in players.items():
        rep = db.get_reputation(uid)
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎰 БИНГО" or text == "БИНГО":
        if not game_active:
            ...
            return
        if game_paused:
            await update.message.reply_text("⏸️ Игра на паузе, ожидается решение администратора по текущему победителю.")
            return
...
async def set_commands(app):
    await app.bot.set_my_commands([
        ...
        BotCommand("endgame", "(админ) Подтвердить/отклонить победителя: /endgame yes|no"),
